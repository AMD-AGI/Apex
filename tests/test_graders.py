...
class TestScoringFormula:
    ...
    def test_speedup_3x(self):
        ...
        assert abs(s - (PTS_COMPILED + PTS_CORRECT + 300.0)) < 1e-6
...
class TestModelResultDataclass:
    ...
    def test_kernel_score_normalized_to_320(self):
        r = ModelResult(model_id="m1", kernel_score=320.0, e2e_throughput_ratio=1.0)
...
class TestDetectKernelType:
    ...
    def test_py_extension(self, tmp_path):
        ...
        f.touch()
        assert kernel_grader._detect_kernel_type(f) == "pytorch"
...
class TestGradeTask:
    ...
    def test_docker_image_param_accepted(self, mock_output_dir):
        task_dir = mock_output_dir / "task_pass"
        with patch.object(kernel_grader, "run_magpie_compare", return_value={"error": "mock"}):
            result = kernel_grader.grade_task(task_dir, docker_image="rocm/pytorch:latest")
