...
from apex.optimization.e2e.preflight import hf_offline
...
def test_hf_offline_hint_is_explicit_boolean(tmp_path: Path) -> None:
    assert hf_offline(_spec(tmp_path, True)) is True
    assert hf_offline(_spec(tmp_path, False)) is False


@pytest.mark.parametrize("value", ["true", 1, None])
def test_hf_offline_hint_rejects_coercion(tmp_path: Path, value: object) -> None:
    with pytest.raises(ContractError) as caught:
        hf_offline(_spec(tmp_path, value))
