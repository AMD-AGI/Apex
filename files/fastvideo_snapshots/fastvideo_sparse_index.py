"""Standalone FastVideo sparse index Triton snapshot for Apex."""
...
def topk_index_to_map(index: torch.Tensor, num_kv_blocks: int) -> torch.Tensor:
    ...
    topk_index_to_map_kernel[(bs, h, num_q_blocks)](
        block_map,
        index,
        block_map.stride(0),
        block_map.stride(1),
...
def map_to_index(block_map: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    map_to_index_kernel[(bs, h, num_q_blocks)](
        block_map,
        index,
        index_num,
        block_map.stride(0),
